...
export default function Toolbar() {
  ...
  async function addHistorySpace(name: string, path: string) {
    ...
    await invoke("write_history_file", {
      data: JSON.stringify(newHistorySpace),
    });
  }
  ...
  return (
    <div
      className={
        "size-full flex justify-between items-center cursor-pointer border-b"
      }
      id={"tool-bar"}
    ...
    >
      <div className={"flex flex-row"} id={"function-area"}>
        <div
          className={"h-7 w-7 flex items-center justify-center ml-2 mr-1"}
          id={"app-icon"}
        >
        ...
        <div className={"h-7"}>
          <RepoSelection
            ...
            addHistorySpace={addHistorySpace}
          />
        </div>
      </div>
      <div
        id={"control-button-group-area"}
        className={"absolute right-0 top-0 h-10 w-auto flex flex-rows"}
      >
        <div
          className={
            "h-10 w-12 flex align-middle justify-center hover:bg-gray-100 active:bg-gray-200"
          }
        ...
        <div
          className={
            "h-10 w-12 flex align-middle justify-center hover:bg-[#e93147] active:bg-[#f1626c]"
          }
